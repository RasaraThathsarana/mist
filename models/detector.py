...
class Detector(torch.nn.Module):
    ...
    def forward(self, images):
        ...
        idx = torch.stack([torch.arange(images.shape[0], device=device).reshape(-1,1,1).repeat(1,self.config.k,heatmap.shape[1]-1),
                           torch.arange(1,heatmap.shape[1], device=device).reshape(1,1,-1).repeat(images.shape[0],self.config.k,1),
...
class ResNet34Detector(torch.nn.Module):
    ...
    def forward(self, images, anchor_size):
        ...
        # print("heatmap_r device:", heatmap_r.device)
        # print("heatmap_x_y device:", heatmap_x_y.device)
        # print("heatmap_w_h device:", heatmap_w_h.device)
...
class CustomResNetDetector(torch.nn.Module):
    ...
    def forward(self, x, anchor_size):
        ...
        N,_,W,H = h_x.shape
        device = h_x.device
        h_x = torch.cat([h_x,torch.zeros(N,2,W,H, device=device),torch.ones(N,2,W,H, device=device)*anchor_size],dim=1)
        return f_x, h_x
